...
38. **Domain errors should not be exit signals** — lower-layer modules (config, model, scaffold, groom) used to raise SystemExit, which is fine for CLI but crashes TUI worker threads silently (SystemExit inherits BaseException, not Exception). Domain exceptions (ConfigError, ModelError, GroomingError) propagate cleanly through both adapters; the CLI catches them in main() and converts to SystemExit(1), while the TUI catches them via except Exception in worker threads. Command functions may now raise GotgError subclasses instead of SystemExit for deep config/model errors — production callers go through main() which handles the conversion
